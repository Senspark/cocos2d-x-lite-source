...
namespace soomla {
    ...
    bool CCMarketItem::initWithDictionary(__Dictionary *dict) {
        ...
        auto item = dict->objectForKey(CCStoreConsts::JSON_MARKET_ITEM_MARKET_PRICE_MICROS);
        if (dynamic_cast<cocos2d::__Integer*>(item) != nullptr) {
            if (mMarketPriceMicros != item) {
                auto i = static_cast<cocos2d::__Integer*>(item);
                setMarketPriceMicros(cocos2d::__Double::create(static_cast<double>(i->getValue())));
            }
        } else {
