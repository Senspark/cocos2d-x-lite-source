...
Ref *CCJsonHelper::getCCObjectFromJson(json_t *obj) {
    ...
    if (json_is_object(obj)) {
        ...
        if (intVal >= std::numeric_limits<int>::min() &&
            ...
            return cocos2d::__Integer::create(static_cast<int>(intVal));
        }

        cocos2d::log("%s: %lld", __PRETTY_FUNCTION__, intVal);

        return cocos2d::__Double::create(static_cast<double>(intVal));
    }
